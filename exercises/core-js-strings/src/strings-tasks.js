...
function countVowels(str) {
  const dict = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
...
function isPalindrome(str) {
  ...
  const initialStr = str
    ...
    .filter((char) => isLetter(char))
    .join();
  return reverseStr.toLowerCase() === initialStr.toLowerCase();
}
...
function findLongestWord(sentence) {
  ...
  const index = wordsLength.indexOf(Math.max(...wordsLength));
  return arr[index];
}
...
function invertCase(str) {
  ...
  return str.split('').reduce((acc, next) => acc + invert(next), '');
}
