...
function makeWord(lettersObject) {
  const resArr = Object.entries(lettersObject).reduce(
    (acc, [key, positions]) => {
...
const cssSelectorBuilder = {
  ...
  addSelectorPart(selectorPart, value, order) {
    if (
      (selectorPart === 'tag' && this.hasTag) ||
      (selectorPart === 'id' && this.hasId) ||
      (selectorPart === 'pseudoElement' && this.hasPseudoElement)
    ) {
      throw new Error(
    ...
    resObj.resStr = this.resStr + value;
    resObj.order = order;
    resObj.hasTag = selectorPart === 'tag' || this.hasTag;
    resObj.hasId = selectorPart === 'id' || this.hasId;
    resObj.hasPseudoElement =
      selectorPart === 'pseudoElement' || this.hasPseudoElement;
    return resObj;
  },

  element(value) {
    return this.addSelectorPart('tag', value, 1);
  },

  id(value) {
    return this.addSelectorPart('id', `#${value}`, 2);
  },

  class(value) {
    return this.addSelectorPart('class', `.${value}`, 3);
  },

  attr(value) {
    return this.addSelectorPart('attr', `[${value}]`, 4);
  },

  pseudoClass(value) {
    return this.addSelectorPart('pseudoClass', `:${value}`, 5);
  },

  pseudoElement(value) {
    return this.addSelectorPart('pseudoElement', `::${value}`, 6);
  },

  combine(selector1, combinator, selector2) {
    const newOrder = Math.max(selector1.order, selector2.order);
    this.checkOrder(newOrder);
    const resObj = Object.create(cssSelectorBuilder);

    const first = selector1.stringify();
    const second = selector2.stringify();

    resObj.resStr = `${first} ${combinator} ${second}`;
    resObj.order = newOrder;
    resObj.hasTag = selector1.hasTag || selector2.hasTag;
    resObj.hasId = selector1.hasId || selector2.hasId;
    resObj.hasPseudoElement =
      selector1.hasPseudoElement || selector2.hasPseudoElement;
    return resObj;
  },

  stringify() {
    const result = this.resStr;
    this.resStr = '';
    return result;
  },
};
